...
X_counts = import_pickle(f"{XXXX_processed_path}X_counts.pkl")

D_appearance = import_pickle(f"{XXXX_processed_path}D_appearance.pkl")
D_disappearance = import_pickle(f"{XXXX_processed_path}D_disappearance.pkl")
...
df_appearance_embeddings_train = pd.read_csv(
    f"{XXXX_processed_path}appearance_embeddings_train.csv", index_col=0
)
df_appearance_embeddings_test = pd.read_csv(
    f"{XXXX_processed_path}appearance_embeddings_test.csv", index_col=0
)
df_disappearance_embeddings_train = pd.read_csv(
    f"{XXXX_processed_path}disappearance_embeddings_train.csv", index_col=0
)
df_disappearance_embeddings_test = pd.read_csv(
    f"{XXXX_processed_path}disappearance_embeddings_test.csv", index_col=0
)
...
Y_train = np.array(
    [[X[(user_id, XXXX_K - 1, tag)] for tag in all_tags] for user_id in all_users]
)
Y_test = np.array([[Y[(user_id, tag)] for tag in all_tags] for user_id in all_users])

X_prev = np.array(
    [[X[(user_id, XXXX_K - 2, tag)] for tag in all_tags] for user_id in all_users]
)
...
make_report(Y_pred, Y_test, Y_train).to_csv(
    f"{XXXX_results_path}Popularity.csv", index=False
)

baseline = NMFBaseline(all_tags, X_counts, XXXX_K)
Y_pred = baseline.predict(all_users)
make_report(Y_pred, Y_test, Y_train).to_csv(
    f"{XXXX_results_path}NMFBaseline.csv", index=False
)
...
make_report(Y_pred, Y_test, X_prev).to_csv(
    f"{XXXX_results_path}Previous.csv", index=False
)
...
Y_pred = baseline.predict(all_users)
make_report(Y_pred, Y_test, Y_train).to_csv(
    f"{XXXX_results_path}PreviousPlug.csv", index=False
)
